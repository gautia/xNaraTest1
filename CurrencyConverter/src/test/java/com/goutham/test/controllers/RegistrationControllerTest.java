...
import static com.goutham.test.TestConstants.ERROR_TYPE_EMAIL;
import static com.goutham.test.TestConstants.FORM_FIELD_ADDRESS_LINE1;
import static com.goutham.test.TestConstants.FORM_FIELD_CITY;
import static com.goutham.test.TestConstants.FORM_FIELD_COUNTRY;
import static com.goutham.test.TestConstants.FORM_FIELD_DOB;
import static com.goutham.test.TestConstants.FORM_FIELD_EMAILID;
import static com.goutham.test.TestConstants.FORM_FIELD_FIRSTNAME;
import static com.goutham.test.TestConstants.FORM_FIELD_PASSWORD;
import static com.goutham.test.TestConstants.FORM_FIELD_USERNAME;
import static com.goutham.test.TestConstants.FORM_FIELD_ZIP_CODE;
import static com.goutham.test.TestConstants.REGISTRATION_FORM;
import static com.goutham.test.TestConstants.TEST_USER_NAME;
import static com.goutham.test.TestConstants.WEB_INF_LOGIN_JSP;
...
public class RegistrationControllerTest {
	...
	public void postRequest_NoParameters_ReturnsRegistrationPageWithErrors() throws Exception {
		...
		// Act
		ResultActions result = mockMvc.perform(MockMvcRequestBuilders.post(CurrencyConverterConstants.REGISTER_REQUEST_MAPPING));

		// Assert
		result.andExpect(status().isOk()).andExpect(model().attributeExists(REGISTRATION_FORM))
				.andExpect(model().hasErrors()).andExpect(view().name(CurrencyConverterConstants.REGISTER_RESPONSE_MAPPING))
	...
	public void getRegister_NoParameters_ReturnsRegistrationPageNoErrors() throws Exception {
		...
		// Assert
		result.andExpect(status().isOk()).andExpect(model().attributeExists(REGISTRATION_FORM))
				.andExpect(model().hasNoErrors()).andExpect(view().name(CurrencyConverterConstants.REGISTER_RESPONSE_MAPPING))
				.andExpect(handler().handlerType(RegistrationController.class))
	...
	public void postRequest_ValidParameters_ForwardsToLoginPageNoErrors() throws Exception {
		...
		// Assert
		result.andExpect(status().isOk()).andExpect(model().attributeExists(REGISTRATION_FORM))
				.andExpect(model().hasNoErrors()).andExpect(view().name(CurrencyConverterConstants.LOGIN_RESPONSE_MAPPING))
				.andExpect(handler().handlerType(RegistrationController.class))
				.andExpect(handler().methodName("handleRegistrationRequest"))
	...
	private ResultActions postRegisterationForm() throws Exception {
		return mockMvc.perform(MockMvcRequestBuilders.post(CurrencyConverterConstants.REGISTER_REQUEST_MAPPING)
	...
	private MultiValueMap<String, String> populateRequestParameters() {
		...
		requestParameters.put(FORM_FIELD_PASSWORD, Arrays.asList(CurrencyConverterConstants.PASSWORD));
